...
describe('contentSecurityPolicy', () => {
  it('is unchanged without additions', () => {
    expect(contentSecurityPolicy('n1')).toBe(
      "default-src 'self'; script-src 'self' 'nonce-n1' https://challenges.cloudflare.com; frame-src https://challenges.cloudflare.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; object-src 'none'",
    );
  });
