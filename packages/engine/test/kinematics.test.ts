    ...
    fc.assert(
      fc.property(
        ...
        (start, dLat, dLon, speedKnots, elapsedMs, capMs, destination) => {
          ...
          if (r.method === 'lane') {
            // Float noise: the path end is the destination to within a nanometre.
            const end = r.remainingPath[r.remainingPath.length - 1]!;
            expect(end.lat).toBeCloseTo(destination.lat, 9);
            expect(end.lon).toBeCloseTo(destination.lon, 9);
          }
        },
      ),
      { numRuns: 300 },
